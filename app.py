...
def endUserMokas():
    ...
    if request.method == 'POST':
        ...
        pinjam11= mw.rupiah_format(pinjam1,True)
        pokok = round(pinjam1/tenor1,2)
        pokok1 = mw.rupiah_format(pokok,True)
        angs = data[(data['Tenor'] == tenor1) & (data['Pinjam']==pinjam1)].index
        try:
            angs2 = data['Angsuran'].iloc[angs].values[0]
            angs22 = mw.rupiah_format(angs2,True)
            bunga = angs2-pokok
            bunga1 = mw.rupiah_format(bunga,True)
            totalAngs = angs2*tenor1
            totalAngs1 = mw.rupiah_format(totalAngs, True)
